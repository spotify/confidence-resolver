...
| `buffer.rs` | In-isolate accumulation, delivered straight to Confidence |
...
| `FLAG_LOG_SINK` | Default `queue`. `buffer` accumulates logs in isolate memory and POSTs straight to Confidence — no transport to provision, nothing billed per record, throughput scales with isolate count, but no durable hand-off. See `src/flag_log/` |

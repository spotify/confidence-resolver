...
| POST | `/v1/flagLogs:ingest` | Receives Logpush trace-event batches when `FLAG_LOG_SINK=logpush` (bearer-authenticated with `FLAG_LOGS_INGEST_TOKEN`) |
...
`src/flag_log/` holds both shipping paths behind one abstraction, selected once
per isolate by `FLAG_LOG_SINK`:

| File | Role |
|------|------|
| `mod.rs` | `Sink` enum + env parsing, shared destination walk, batch apply-dedup |
| `queue.rs` | Queue producer and the queue consumer |
| `shards.rs` | Shard discovery and round-robin failover |
| `logpush.rs` | `base64(gzip(protobuf))` wire format, inline console emit, extraction |
| `ingest.rs` | HTTP route Logpush POSTs trace-event batches back to |

Queue bindings are created under either sink, so flipping `FLAG_LOG_SINK` back
to `queue` is an immediate rollback.
...
| `FLAG_LOG_SINK` | Default `queue`. Set to `logpush` to ship flag logs via `console.log` → Logpush → `/v1/flagLogs:ingest` instead of the queue. See `src/flag_log/` |
| `RESOLVE_TOKEN_ENCRYPTION_KEY` | (secret) AES-128 key for resolve token encryption; auto-generated by deployer |
| `CONFIDENCE_ACCOUNT_ID` | Account id used when delivering flag logs; set by the deployer |
| `FLAG_LOGS_INGEST_TOKEN` | (secret) Bearer token Logpush presents on the ingest route; auto-generated by deployer |

...
With `FLAG_LOG_SINK=buffer`, the preflight also prints an explicit advisory: the state-only measurement does not fill the log buffer or exercise concurrent deliveries. Those can consume tens of MiB under backend backpressure, so passing the baseline gate is not evidence that buffer mode fits under load. The report includes the selected sink, including `--var` overrides. Queue-consumer batches and deduplication also require request-time headroom. Thresholds remain unchanged until representative load measurements justify a different budget.

...
To skip exposure collection for a single evaluation, pass `_confidence_skip_apply: true` in the evaluation context:
...
| --- | --- | --- | --- |
| `disableExposureCollection` provider option | All OpenFeature evaluations through this provider | Never queued; no deferred apply token is returned | Still sent |

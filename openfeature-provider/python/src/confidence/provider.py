...
class ConfidenceProvider(AbstractProvider):
    ...
    def _resolve_object(
        ...
        try:
            ...
            # Build process request — use simple Resolve if store is configured,
            # otherwise ResolveWithMaterializations (empty, complete).
            if self._materialization_store is not None and not isinstance(
                self._materialization_store, UnsupportedMaterializationStore
            ):
                request = wasm_api_pb2.ResolveProcessRequest()
                request.resolve.CopyFrom(resolve_req)
            else:
                request = wasm_api_pb2.ResolveProcessRequest()
                request.resolve_with_materializations.resolve_request.CopyFrom(
                    resolve_req
                )
    ...
    def _resolve_with_materialization(
        ...
        for _ in range(max_retries):
            with self._resolver_lock:
                response = self._resolver.resolve_process(current_request)
            ...
            if response.HasField("suspended"):
                ...
                current_request = wasm_api_pb2.ResolveProcessRequest()
                current_request.resume.state = response.suspended.state
                for mat in materializations:
                    current_request.resume.materializations.append(mat)
                continue

            raise RuntimeError("Unexpected empty resolve response")

        raise RuntimeError(
            f"exceeded maximum retries ({max_retries}) for handling missing materializations"
        )

...
func evaluate[T any](
	...
	// Create ResolveProcess request with empty complete materializations.
	// This ensures the resolver never suspends — flags with materialization
	// requirements fall through to non-materialized rules.
	// When materialization support is enabled, the materializationSupportedResolver
	// wrapper overrides this with proper suspend/resume handling.

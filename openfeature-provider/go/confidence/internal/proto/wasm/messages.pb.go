...
type SetResolverStateRequest struct {
	...
	// Skip apply/assignment logging entirely. When true, WASM never enqueues
	// FlagAssigned events — the assign queue stays empty. Provider skipApply
	// config is forwarded here on every setResolverState.
	SkipApply     bool `protobuf:"varint,5,opt,name=skip_apply,json=skipApply,proto3" json:"skip_apply,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *SetResolverStateRequest) GetSkipApply() bool {
	if x != nil {
		return x.SkipApply
	}
	return false
...
const file_confidence_wasm_messages_proto_rawDesc = "" +
	...
	"\x12enable_apply_dedup\x18\x04 \x01(\bR\x10enableApplyDedup\x12\x1d\n" +
	"\n" +
	"skip_apply\x18\x05 \x01(\bR\tskipApply\"\x1d\n" +
	"\aRequest\x12\x12\n" +

...
func TestLocalResolverProvider_ReturnsDefaultOnError(t *testing.T) {
	...
	openfeature.SetProviderAndWait(NewLocalResolverProvider(resolverSupplier, stateProvider, mockFlagLogger, nil, "test-secret", slog.New(slog.NewTextHandler(os.Stderr, nil))))
...
func TestLocalResolverProvider_ReturnsCorrectValue(t *testing.T) {
	...
	openfeature.SetProviderAndWait(NewLocalResolverProvider(resolverSupplier, stateProvider, mockFlagLogger, nil, "mkjJruAATQWjeY7foFIWfVAcBWnci2YF", slog.New(slog.NewTextHandler(os.Stderr, nil))))
	client := openfeature.NewClient("test-client")
...
func TestLocalResolverProvider_MissingMaterializations(t *testing.T) {
	...
	t.Run("Provider returns missing materializations error message for UnsupportedMaterializationStore", func(t *testing.T) {
		...
		} else if error.Error() != "error code: GENERAL: resolve failed: failed to read materializations: materialization read not supported" {
			t.Errorf("Expected 'GENERAL: resolve failed: failed to read materializations: materialization read not supported', got: %v", error.Error())
		}

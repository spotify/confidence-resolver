...
/// Managed values win on exact-path collisions, making server configuration
/// authoritative. When both sides have a Struct for the same key, merge
...
mod tests {
    ...
    #[test]
    fn managed_overrides_request_and_fills_missing() {
        ...
        assert_eq!(
            result.fields.get("platform").and_then(|v| v.kind.as_ref()),
            Some(&Kind::StringValue("ios".to_string())),
            "managed should override request"
        );
        let app = match result.fields.get("app").and_then(|v| v.kind.as_ref()) {
            Some(Kind::StructValue(s)) => s,
            _ => panic!("expected managed app struct to be present"),
        };
        assert_eq!(
            app.fields.get("name").and_then(|v| v.kind.as_ref()),
            Some(&Kind::StringValue("default-app".to_string())),
            "managed fills in fields not in request"
        );
    }

...
/// Encode a prost 0.12 Message into a buffer.
/// Exposed so downstream crates using a different prost version can still
/// encode messages generated by this crate.
pub fn encode_message<M: prost::Message>(msg: &M, buf: &mut Vec<u8>) {
    msg.encode(buf).expect("encoding should not fail");
}
